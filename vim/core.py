...
def train(args):
    ...
    with tf.variable_scope('train'):
        ...
        learning_rate = tf.train.exponential_decay(
            init_learning_rate, global_step=global_step, decay_steps=50000, decay_rate=5e-7 * hvd.size())
    ...
    #             else:   # local machine
    #                 (batch_x, batch_y) = next(train_gen.generate()) # train_gen.generate().__next__()    # next(train_gen.generate())
    #                 batch_x_mfcc, batch_y_mfcc, batch_seq_len = tasksmq.batch_wav_to_mfcc_parallel(batch_x, batch_y, agumentation=True)
    ...
    #             # Save model
    #             if (hvd.rank() == 0) and (num_steps > old_num_steps + 1000):
